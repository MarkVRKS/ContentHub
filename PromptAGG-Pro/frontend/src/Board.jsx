      ...
      <div className="glass-effect w-full max-w-3xl rounded-[48px] shadow-[0_40px_100px_var(--accent-glow)] overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-10 flex-1 flex flex-col gap-6 overflow-hidden">
          ...
          <button 
            ...
            className="w-full py-6 text-white rounded-3xl font-black text-xs uppercase tracking-[0.3em] hover:scale-[1.02] active:scale-95 transition-all shadow-[0_20px_40px_var(--accent-glow)] flex items-center justify-center gap-4 group"
          >
...
const Board = memo(function Board({ projectId, posts, refreshPosts, projectContext }) {
    ...
    try {
      ...
      const promises = postsArray.map(post =>
        api.createPost({
          ...
          platforms: post.platforms || {} // ИСПРАВЛЕНИЕ: Больше нет жесткой привязки к VK
        })
      );
    ...
    <div className="w-full h-full flex flex-col min-w-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 items-center px-6 md:px-12 py-8 bg-[var(--bg-card)]/50 backdrop-blur-xl border-b-2 border-[var(--border-main)] sticky top-0 z-[40] gap-6 transition-colors duration-500">
        <div className="flex items-center gap-6">
          ...
          <div className="text-left">
            <h2 key={monthName} className="text-4xl font-black text-[var(--text-main)] uppercase tracking-tighter leading-none animate-in slide-in-from-top-4">
              {monthName}
            </h2>
          ...
          <div className="flex items-center bg-[var(--bg-card)] p-2 rounded-full border-2 border-[var(--border-main)] shadow-xl backdrop-blur-md">
            <button onClick={() => setViewDate(new Date())} className="flex items-center gap-4 pl-2 pr-6 py-2 hover:bg-[var(--bg-input)] rounded-full transition-all group">
              <div 
                ...
                className="w-12 h-12 text-white rounded-full flex items-center justify-center font-black text-base shadow-md group-hover:scale-110 transition-transform"
              >
                {todayNum}
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-[120px] text-center">
                <span className="text-base font-black uppercase tracking-[0.3em] text-[var(--text-main)]">
                  {monthName}
                </span>
              </div>
        ...
        <div className="flex justify-end">
          <button 
            ...
            style={{ backgroundImage: 'linear-gradient(to right, var(--grad-1), var(--grad-2))' }}
            className="flex items-center gap-4 px-8 py-5 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-[0_15px_30px_var(--accent-glow)] group"
          >
        ...
        {days.map(day => {
          ...
          return (
            <div
              ...
              style={{ borderColor: isToday ? 'var(--accent)' : 'var(--border-main)', boxShadow: isToday ? '0 0 40px var(--accent-glow)' : 'none' }}
              className={`w-full bg-[var(--bg-card)] border-2 rounded-[48px] flex flex-col items-center p-8 cursor-pointer hover:border-[var(--border-hover)] hover:-translate-y-2 transition-all duration-300 relative group ${isToday ? 'ring-4 ring-[var(--accent-glow)]' : ''}`}
            >
              <span 
              ...
              <span 
                style={{ color: isToday ? 'var(--accent)' : 'var(--text-main)' }}
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-auto w-full">
                {platformsInfo.map(plat => (
                  <button
                    key={plat.id}
                    onClick={(e) => handleBadgeClick(e, day, plat.id, dayPosts)}
                    className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${activePlats.has(plat.id) ? plat.activeClass : "bg-[var(--bg-input)] text-[var(--text-muted)] border border-[var(--border-main)] opacity-40 hover:opacity-100 hover:border-[var(--border-hover)]"}`}
                  >
                    {plat.label}
                  </button>
                ))}
      ...
      {selectedDay && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-4xl glass-effect rounded-[56px] shadow-[0_50px_100px_rgba(0,0,0,0.4)] flex flex-col max-h-[92vh] overflow-hidden animate-in zoom-in-95 duration-500">
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-12 bg-transparent ideas-scroll">
              ...
              {/* ИСПРАВЛЕНИЕ: Теперь пост создается с пустым объектом platforms */}
              <button
